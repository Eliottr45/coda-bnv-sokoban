...
void déplacement(char ** grille, position * pos, position * box)
{
    ...
    printf("Choisissez une direction où vous déplacer: \nHaut (z) \nBas (s) \nGauche (q) \nDroite (d)\n");
    scanf(" %c", &way);
    ...
    if(way == 'z')
    {
        if(pos->x - 1 < 0) //vérification de la possiblité de déplacement
        {
            printf("Vous ne pouvez pas aller vers le haut, il y a un mur. \n");
        }
        else if(pos->x - 1 == box->x && pos->y == box->y)
        {
            if (box->y - 1 < 0 || grille[box->x - 1][box->y] != ' ' && grille[box->x - 1][box->y] != '.')
            {
    ...
    else if(way == 's')
    {
        ...
        {
            if (box->y + 1 > 7 || grille[box->x + 1][box->y] != ' ' && grille[box->x + 1][box->y] != '.')
            {
                printf("Impossible de pousser la box.\n");
            }
            else
    ...
    else if(way == 'q')
    {
        ...
        {
            if (box->x - 1 < 0 || grille[box->x][box->y - 1] != ' ' && grille[box->x][box->y - 1] != '.')
            {
                printf("Impossible de pousser la box.\n");
            }
            else
    ...
    else if(way == 'd')
    {
        ...
        {
            if (box->x + 1 > 7 || grille[box->x][box->y + 1] != ' ' && grille[box->x][box->y + 1] != '.')
            {
                printf("Impossible de pousser la box.\n");
            }
            else
